...
import { reqProductPage, reqSearchProduct } from "../../api";
...
export default class ProductHome extends Component {
  ...
  getProducts = async (pageNumber) => {
    this.setState({ onLoading: true });
    const result = await reqProductPage(pageNumber, 5);
    this.setState({ onLoading: false });
    if (result.status === 0) {
      // this.setState({ products: result.data.list, total: result.data.total });
    }
  };
  ...
  render() {
    const { products, total, onLoading, productDesc, productName } = this.state;
    const title = (
      <span>
        <Select
          value={productName}
        ...
        ></Input>
        <Button type="primary" onClick={() => this.search(1)}>
          搜索
        </Button>
      </span>
    );
